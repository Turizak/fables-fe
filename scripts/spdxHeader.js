...
function validateDirectory(directory) {
  if (!Array.isArray(directory)) {
    throw new Error("targetDirectory must be an array of strings");
  }
...
function addHeaderToFiles(directories) {
  let modifiedFiles = 0;
  let skippedFiles = 0;
      ...
      entries.forEach((entry) => {
        ...
        try {
          if (entry.isDirectory()) {
            const { modified, skipped } = addHeaderToFiles([fullPath]); // Recursively call with subdirectory
